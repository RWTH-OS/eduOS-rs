...
__udivdi3:
	push ebx
	mov	ebx, [esp+20]  ; Find the index i of the leading bit in b.
	...
	mov eax, [esp+8]   ; to [1] if the high word is larger than bhi
	...
	sbb edi, 0         ; decrement q if remainder is negative
	xor edx, edx
	mov eax, edi
	pop edi
	pop ebx
...
L1: ; High word of a is greater than or equal to (b >> (1 + i)) on this branch
...
L9: ; High word of b is zero on this branch
	...
	div ecx
	mov ebx, eax
	mov eax, [esp+8]    ; Find qlo such that
	div ecx
	mov edx, ebx        ; rhi:alo = qlo*b + rlo  with 0 ≤ rlo < b
	pop ebx
	ret                 ; and return qhi:qlo
