...
use crate::arch::aarch64::kernel::irq;
...
extern "C" fn entry_point(func: extern "C" fn()) -> ! {
	...
	// does not restore PSTATE. As the very first task is started from within the
	// timer interrupt handler, IRQs are still masked here. Enable them so that
	// this task can be preempted as well.
	irq::irq_enable();
	...
	fn create_stack_frame(&mut self, func: extern "C" fn()) {
		...
		unsafe {
			...
			*stack = 0xDEADBEEFu64;
			...
			// The cooperative `switch` pops the saved state (16 register pairs)
			// before `ret`, leaving `sp = last_stack_pointer + 256`. Keep the
			// frame 16-byte aligned so the new task starts with an aligned stack
			// pointer (required by the ABI and the SP alignment check).
			stack = (stack as usize & !0xfusize) as *mut u64;
