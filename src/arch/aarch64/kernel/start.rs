...
unsafe fn pre_init() -> ! {
	...
	*/
	...
	/* Ensure the system control register update is in effect. */
	unsafe {
		asm!("isb", options(nostack));
	}

	/*
	 * Install the exception vector table, bring up the GICv3 interrupt
	 * controller and start the periodic timer. Interrupts remain masked
	 * until `main` enables them explicitly.
	 */
	crate::arch::init();
