...
For macOS is required that Apple's *Command Line Tools* and the package manager [Homebrew](https://brew.sh) are installed.
After installing *Homebrew*, install the required tools *wget*, *nasm* and *qemu* to buil eduOS-rs with following command.

```sh
$ brew install wget qemu nasm
```

In addition, we have to install [binutils](https://www.gnu.org/software/binutils/) to support the *Executable and Linkable Format* (ELF), which is the link format of our kernel.
...
We need to install the Rust toolchain, to check out the sources and rebuild the Rust runtime using a
bare-metal target and without hardware floating point support.
